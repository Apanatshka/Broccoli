...
pub fn plot_pendulum(b_output: BroccoliOutput, initial_states: &[Vec<f64>]) {
    let pendulum_env = EnvironmentPendulum::new();
    if let Some(score) = b_output.score {
        ...
        for trace in traces.iter().enumerate() {
            for f in 0..trace.1[0].len() {
                ...
                let y_name = pendulum_env.environment_info().feature_name(f);
